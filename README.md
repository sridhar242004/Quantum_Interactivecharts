...
<!-- ─── BADGES ROW 1 — STATUS ─────────────────────────────── -->
[![Live](https://img.shields.io/badge/🌐_Live_Demo-quantuminteractivecharts.netlify.app-00e5ff?style=for-the-badge)](https://quantuminteractivecharts.netlify.app/)
...
![Plotly](https://img.shields.io/badge/Plotly-Latest-3f4f75?style=flat-square&logo=plotly&logoColor=white)
![Three.js](https://img.shields.io/badge/Three.js-r128_WebGL-black?style=flat-square&logo=three.js&logoColor=white)
![D3](https://img.shields.io/badge/D3.js-v7-f9a03c?style=flat-square&logo=d3.js&logoColor=white)
![Bootstrap](https://img.shields.io/badge/Bootstrap-5.3-7952B3?style=flat-square&logo=bootstrap&logoColor=white)
![Tailwind](https://img.shields.io/badge/Tailwind-CDN-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white)
...
![Algorithms](https://img.shields.io/badge/Algorithms-15_Total-f43f5e?style=flat-square)
![Zero Backend](https://img.shields.io/badge/Backend-Zero_(Pure_Client)-%2300e5ff?style=flat-square)
...
graph TB
    subgraph Browser["🌐 Browser — 100% Client Side"]
        ...
        subgraph Render["🎨 Render Layer"]
            PLOTLY[Plotly.js Traces]
            D3[D3.js Force Graphs]
            CHARTJS[Chart.js Hero]
            THREEJS[Three.js WebGL]
        end

        CSV[📄 CSV FileReader] --> STATE
        STATE --> ALGO
        ALGO --> Render
        UI --> STATE
    end

    subgraph Cloud["☁️ Groq Cloud"]
        LPU[LPU Inference Engine]
        LLM[llama-3.3-70b-versatile]
        LPU --> LLM
    end

    STATE -- "POST /openai/v1/chat/completions (SSE stream)" --> LPU
    LLM -- "token stream" --> DEMO

    style Browser fill:#03030d,stroke:#00e5ff,color:#e8eeff
    style Cloud fill:#1a0a2e,stroke:#a855f7,color:#e8eeff
    style Engine fill:#0a0c23,stroke:#00e5ff,color:#e8eeff
    style Render fill:#0a0c23,stroke:#a855f7,color:#e8eeff
    style UI fill:#0a0c23,stroke:#00ff88,color:#e8eeff
```
...
```mermaid
flowchart LR
    A[📁 CSV File\nDrop or click] --> B[FileReader\nreadAsText]
    B --> C[Parser\nHeaders · Types\nRow objects]
    C --> D[STATE.currentData\nheaders + data array]

    D --> E{Algorithm\nSelected}

    E -->|PCA| F1[Power Iteration\nGram-Schmidt]
    E -->|K-Means| F2[K-Means++\nLloyd iterations]
    E -->|DBSCAN| F3[ε-neighborhood\ngraph]
    E -->|IsoForest| F4[60-tree\nensemble]
    E -->|LinReg| F5[Closed-form\nOLS]

    F1 & F2 & F3 & F4 & F5 --> G[STATE.processedData\nresult object]

    G --> H[VizEngine.render]
    G --> I[buildSystemPrompt\ncontext injection]
    ...
    style A fill:#0d1117,stroke:#00e5ff,color:#c8d0e7
    style L fill:#0d1117,stroke:#a855f7,color:#c8d0e7
    style M fill:#0d1117,stroke:#00ff88,color:#c8d0e7
```
...
```mermaid
graph LR
    subgraph Trick["Counter-Rotation Trick"]
        direction TB
    ...
    style Trick fill:#0a0c23,stroke:#6366f1,color:#c8d0e7
```
